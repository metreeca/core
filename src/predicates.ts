...
/**
 * Composable predicate functions and combinators for filtering operations.
 *
 * Provides composable predicate functions for use with
 * {@link https://developer.mozilla.org/docs/Web/JavaScript/Reference/Global_Objects/Array/filter | Array.filter()}
 * and other predicate-based operations.
 *
 * **Usage**
 *
 * ```typescript
 * import { not, and, or } from '@metreeca/core/predicates';
 *
 * // Define simple predicates
 *
 * const isEven = (n: number) => n % 2 === 0;
 * const isPositive = (n: number) => n > 0;
 * const isLarge = (n: number) => n > 100;
 *
 * // Negate a predicate
 *
 ...
 * values.filter(and(isPositive, isEven)); // [2, 4]
 *
 * // Combine with OR
 *
 * const nums = [1, 50, 101, -10, 150];
 * nums.filter(or(isLarge, (n) => n < 0)); // [101, -10, 150]
 *
 * // Complex compositions
 *
 * const data = [-5, -2, 1, 2, 3, 4, 50, 101];
 * data.filter(and(
 *   isPositive,
 *   or(isEven, isLarge)
 * )); // [2, 4, 50, 101]
 ...
 * ];
 *
 * const isAdult = (u: typeof users[0]) => u.age >= 18;
 * const isActive = (u: typeof users[0]) => u.active;
 *
 * users.filter(and(isAdult, isActive)); // [{ name: 'Alice', ... }]
 * ```
 *
 * @module
 */
