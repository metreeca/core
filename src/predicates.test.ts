...
describe("not()", () => {
	...
	it("should work with array filter", () => {
		...
		const isEven = (n: number) => n%2 === 0;
		const values = [1, 2, 3, 4, 5].filter(not(isEven));
...
describe("predicate composition", () => {
	...
	it("should compose not with and", () => {
		...
		const values = [-3, -2, -1, 0, 1, 2, 3].filter(isOddAndPositive);

		expect(values).toEqual([1, 3]);

	});

	it("should compose not with or", () => {

		const isEven = (n: number) => n%2 === 0;
		const isNegative = (n: number) => n < 0;
		const isOddOrNegative = or(not(isEven), isNegative);

		const values = [-2, -1, 0, 1, 2, 3].filter(isOddOrNegative);

		expect(values).toEqual([-2, -1, 1, 3]);

	});

	it("should compose and with or", () => {
		...
		const values = [-4, -3, -2, -1, 0, 1, 2, 3, 4].filter(complex);
...
describe("edge cases", () => {
	...
	it("should handle string predicates", () => {
		...
		const values = ["apple", "art", "banana", "ant"].filter(and(startsWithA, hasLength3));
	...
	it("should handle object predicates", () => {
		...
		const values = users.filter(and(isAdult, nameStartsWithA));
