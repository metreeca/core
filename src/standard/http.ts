 ...
 *
 * Provides named constants for HTTP status codes, so that response handling reads as intent rather than as bare
 * numeric literals. Coverage is complete for the codes defined by RFC 9110 § 15, extended with the registered codes in
 * common use defined by later specifications; placeholders reserved without a name and codes registered by narrower
 * protocol extensions are left out and compared as plain numbers.
 *
 * **Usage**
 *
 * ```typescript
 * import { NotFound } from "@metreeca/core/http";
 *
